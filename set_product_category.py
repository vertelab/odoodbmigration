...
for source_template_id in source.env['product.template'].search([]):
    source_template = source.env['product.template'].read(source_template_id, ['product_categ_ids'])
    target_categ_ids = [ get_target_record_from_id('product.public.category', categ_id).id for categ_id in source_template['product_categ_ids'] ] 
    target_pricelist_item.product_categ_ids = [(6, 0, target_categ_ids)]
